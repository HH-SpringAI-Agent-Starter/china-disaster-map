...
.tip-ex { color:#C9A227 }
.extlink { color:#7FB4D6;text-decoration:underline }
.extlink:hover { color:#A9D6F0 }
...
<header>
  ...
  <div class="controls">
    ...
    <button class="btn" id="bRefresh" title="重新拉取在线灾害事件数据（disaster_data.json），并同步刷新年份 / 类型下拉框">更新数据</button>
...
<div class="panel" id="natPanel" style="display:none">
  ...
  <div class="lead" style="margin-top:8px">📄 独立可分享排名页（含完整切换 / 分页 / 导出 CSV）：<a class="extlink" href="city_ranking.html" target="_blank" rel="noopener">city_ranking.html</a></div>
...
document.querySelectorAll('.city').forEach(bindTip);

// 数据加载：优先在线，失败回退内置；可手动触发（更新数据按钮）
function refreshData(){
  const curY = fYear.value, curM = fMonth.value, curT = fType.value;
  return fetch(DATA_URL).then(r => r.ok ? r.json() : Promise.reject()).then(d => {
    DATA = d;
    buildControls();  // 在线数据可能含新年份 / 新类型，重建下拉框
    fYear.value  = [...fYear.options].some(o => o.value === curY) ? curY : 'all';
    fMonth.value = [...fMonth.options].some(o => o.value === curM) ? curM : 'all';
    fType.value  = [...fType.options].some(o => o.value === curT) ? curT : 'all';
...
document.querySelectorAll('.mapcard svg').forEach(svg => {
  svg.addEventListener('click', () => {
    const inner = document.getElementById('mapZoomInner');
    inner.innerHTML = svg.outerHTML;
    inner.querySelectorAll('.city').forEach(bindTip);  // 全屏视图也支持悬停查看城市详情
